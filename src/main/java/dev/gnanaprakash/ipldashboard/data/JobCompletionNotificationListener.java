...
public class JobCompletionNotificationListener implements JobExecutionListener {
    ...
    public void afterJob(JobExecution jobExecution) {
        if (jobExecution.getStatus() == BatchStatus.COMPLETED) {
            ...
            em
                    .createQuery(
                            "SELECT m.team1, COUNT(m) FROM Match m GROUP BY m.team1",
                            Object[].class)
                    .getResultList()
                    .stream()
            ...
            em.createQuery(
                            "SELECT m.matchWinner, COUNT(m) from Match m GROUP BY m.matchWinner",
                            Object[].class)
                    .getResultList()
                    .forEach(
                            e -> {
                                ...
                                Optional.ofNullable(teamData.get(teamName))
                                        .ifPresent(team -> team.setTotalWins(totalWins));
                            });
